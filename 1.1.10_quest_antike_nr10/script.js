...
function renderLives(){
  ...
  c.innerHTML=""; for(let i=0;i<3;i++){ const s=document.createElement("span");
    s.textContent="✏️"; s.classList.add("pen"); if(i>=lives) s.classList.add("lost"); c.appendChild(s); }
}
function loseLife(){
  if(lives>0){ lives--; localStorage.setItem("lives", lives); renderLives(); }
  if(lives<=0){ setTimeout(()=>{ alert("Game Over! Keine Leben mehr ✏️");
    localStorage.setItem("lives","3"); window.location.href="../0_Homepage_Basic/index.html"; },300); }
}
...
function checkAnswer(answer, btn){
  ...
  else{ btn.classList.add("button-red");
...
function getScriptBase(){ const s=[...document.scripts].find(x=>x.src&&x.src.endsWith("script.js"));
  return new URL(".", s ? s.src : document.location.href); }
const SCRIPT_BASE=getScriptBase();
function toAbsolute(p){ return new URL(p, SCRIPT_BASE).href; }
function normalizeUrl(h){ const u=new URL(h, SCRIPT_BASE); u.hash=""; u.search="";
  let path=u.pathname.replace(/\/index\.html$/,"/"); if(!path.endsWith("/")) path+="/"; return u.origin+path; }
function shuffleArray(a){ for(let i=a.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [a[i],a[j]]=[a[j],a[i]]; } }
function buildOrder(){ const abs=RAW_QUESTIONS.map(toAbsolute); const order=abs.slice(); shuffleArray(order);
  localStorage.setItem(ORDER_KEY, JSON.stringify(order)); localStorage.setItem(INDEX_KEY,"0"); return order; }
...
if(nextBtn){
  nextBtn.addEventListener("click",()=>{
    ...
    if(i>=order.length){ alert("Quiz beendet! Sie haben alle Fragen beantwortet."); clearQuizProgress(); window.location.href="../0_Homepage_Basic/index.html"; return; }
    setIndex(i); window.location.href=order[i];
  });
}

// Initial UI
renderLives(); updateXPDisplay();
